...
;; 4 sheets of plywood, han tänkte 3 från början, men teensy fick inte plats, Han hade 0.19 inch plywood, istället för .25 inch, vilket var vad han hade tänkt sig.

;; .25 * 3 * 2.51 = 1.8824 cm
;; .19 * 4 * 2.51 = 1.9076 cm

;; OBS: han tweekade den lite, för att göra hålen större, för hans CNC maskin är inte helt perfekt.

;; signature plastics, PBT, DSA.

;; set output quality
(def fn 50)

;; Distance between key centers.
(def column-spacing 19)
(def row-spacing column-spacing)
...
;; the top plate.
(def key-hole-size 20)

;; rotation angle, the angle between the halves is twice this number
(def angle 10)

;; The radius of screw holes. Holes will be slightly bigger due to the
;; cut width
(def screw-hole-radius 1.5)
...
(def washer-radius (* 4 screw-hole-radius))
...
;; of 10 with angle=0.
(def back-screw-hole-offset 0)

;; Distance between halves.
(def hand-separation 0)

;; The approximate size of switch holes. Used to determine how thick
;; walls can be, i.e. how much room around each switch hole to leave.
;; See 'spacer'
(def switch-hole-size 14.3)

;; Sets whether the case should use notched holes. As far as I can
;; tell these notches are not all that useful...
(def use-notched-holes false)

;; Number of rows and columns in the matrix. You need to update
;; staggering-offsets if you change n-cols.
(def n-rows 4)
(def n-cols 6)

;; The width of the USB cable hole in the spacer.
(def cable-hole-width 4)

;; Vertical column staggering offsets. The first element should be
;; zero. Starting from center, moving out
(def staggering-offsets  '(0 5 11 6 3 0))


;; Rotate children `angle` degrees around `center`.
(defn rz [angle center]
  (->> (translate (- center))
       (rotate angle)
       (translate [center])))
...
module rz(angle, center=undef) {
 ...
 ;; USB cable hole.
(defn rz-fun [p angle center]
  "rz_fun"
  (let [delta-0 (- (first p) (first center))
        delta-1 (- (second p) (second center))
        cos     (Math/cos angle)
        sin     (Math/sin angle)]
    (list (+ (- (* cos delta-0) (* sin delta-1))
             (first center))
...
module rotate_half() {
  ...
  ;; it is shifted 0.5*column_spacing up relative to the nearest
  ;; column.
...
module screw_hole(radius, offset_radius, position, direction) {
  ...
  ;; radius_offset is the offset in the x (or y) direction so that we're offset_radius from position
...
(translate [320 150] (spacer))
(translate [320 0] (switch-plate))
(translate [0 150] (bottom_plate))

(spit "things/atreus_tumme.scad"
      (write-scad atreus))
